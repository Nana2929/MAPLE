...
class FastRandomIdentitySampler(Sampler):
    def __init__(
        ...
        self.instance_per_class = (
            int(np.ceil(self.num_instances / self.select_instances))
    ...
    def __iter__(self):
        ...
        for k, v in self.index_dic.items():
            ...
            if replace:
                v.extend(
                    np.random.choice(
                        v,
                        size=self.instance_per_class - self.num_instances,
                        replace=False,
                    ).tolist()
                )
                v = np.asarray(v)
        ...
        for d in range(devide):
            for i in indices:
                this = self.index_dic[int(i)][
                    d * self.select_instances : (d + 1) * self.select_instances
                ]
...
class ClassAwareSampler(Sampler):
    ...
    def __init__(
        self, nclass, cls_data_list, gt_labels, seed, num_samples_cls=3, reduce=4
    ):
        random.seed(seed)
        torch.manual_seed(seed)
        # nclass = len(np.unique(data_source.CLASSES))
        self.cls_data_list = cls_data_list
        self.gt_labels = gt_labels
        self.epoch = 0

        self.class_iter = RandomCycleIter(range(nclass))
        """
        ...
        self.num_samples = int(
            max([len(x) for x in self.cls_data_list]) * len(self.cls_data_list) / reduce
