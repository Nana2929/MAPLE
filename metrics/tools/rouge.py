...
def rouge_n(evaluated_sentences, reference_sentences, n=2):
    ...
    if isinstance(evaluated_sentences[0], str):
    ...
    # print("N:", n)
    # print("evaluated_ngrams:", evaluated_ngrams)
    # print("reference_ngrams:", reference_ngrams)
...
def rouge(hypotheses, references):
    ...
    hyps_and_refs = [_ for _ in hyps_and_refs if len(_[0]) > 0 and len(_[1]) > 0]
    hypotheses, references = zip(*hyps_and_refs)

    # Calculate ROUGE-1 F1, precision, recall scores
    rouge_1 = [rouge_n(hyp, ref, 1) for hyp, ref in zip(hypotheses, references)]
    # add equal number of zeros for the filtered cases
    rouge_1 += [(0, 0, 0)] * (len(hypotheses) - len(rouge_1))
    ...
    rouge_2 += [(0, 0, 0)] * (len(hypotheses) - len(rouge_2))
    ...
    rouge_l += [(0, 0, 0)] * (len(hypotheses) - len(rouge_l))
