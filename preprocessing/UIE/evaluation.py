...
class HuggingfacePredictor:
    ...
    def predict(self, text):
        ...
        inputs = self._tokenizer(
            text, padding=True, return_tensors='pt', max_length=self._max_source_length).to(self._model.device)
...
def main():
    ...
    # the relative package path is assumed to be UIE/ ()
    parser.add_argument(
        '--data', '-d', default='data/text2spotasoc/absa/14-15-16res')
    parser.add_argument(
        '--model', '-m', default='../uie_finetune_checkpoints/run1')
    ...
    for split, split_name in [('val', 'eval'), ('test', 'test')]:
        ...
        print("Predicting on: ", gold_filename)
        print("[sample] text:", text_list[0])
        print("[sample] tokens", token_list[0])
        ...
        save_dir = os.path.join("output", options.data)
        os.makedirs(save_dir, exist_ok=True)
        with open(os.path.join(save_dir, f'{split_name}_preds_record.txt'), 'w') as output:
            for record in records:
                output.write(f'{json.dumps(record)}\n')
        with open(os.path.join(save_dir, f'{split_name}_preds_seq2seq.txt'), 'w') as output:
            for pred in predict:
                output.write(f'{pred}\n')

        with open(os.path.join(save_dir, f'{split_name}_results.txt'), 'w') as output:
